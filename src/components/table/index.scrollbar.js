 ...
 * @returns
 */
function autoMove(t, range, oldRange) {
    if (!range)
        return;
    ...
    if (viewport && _selector) {
        ...
        if (_vScrollbar) {
            const totalHeight = (start, maxStart, maxEnd) => {
            ...
            if (oldRange) {
                ...
                if (range.endRow === oldRange.endRow) {
                    ...
                    if (range.startRow < oldRange.startRow) {
                        // up+
                        if (range.startRow > range2.endRow &&
                        ...
                        if (range.startRow >= range4.endRow) {
                            _vScrollbar.scrollBy(totalHeight(range4.startRow, range4.endRow, range.startRow));
                        }
                    }
                }
                else if (range.startRow === oldRange.startRow) {
                    ...
                    if (range.endRow > oldRange.endRow) {
                        ...
                        if (range.endRow >= range4.endRow) {
                            _vScrollbar.scrollBy(totalHeight(range4.startRow, range4.endRow, range.endRow));
                        }
                    }
                ...
                else if (range.startRow > range2.endRow &&
                    range.startRow < range4.startRow) {
                    _vScrollbar.scrollBy(-t.rowsHeight(range.startRow, range4.startRow));
                }
            }
        ...
        if (_hScrollbar) {
            const totalWidth = (start, maxStart, maxEnd) => {
            ...
            if (oldRange) {
                ...
                if (range.endCol === oldRange.endCol) {
                    ...
                    if (range.startCol < oldRange.startCol) {
                        // left+
                        if (range.startCol > range2.endCol &&
                        ...
                        if (range.startCol >= range4.endCol) {
                            _hScrollbar.scrollBy(totalWidth(range4.startCol, range4.endCol, range.startCol));
                        }
                    }
                }
                else if (range.startCol === oldRange.startCol) {
                    ...
                    if (range.endCol > oldRange.endCol) {
                        ...
                        if (range.endCol >= range4.endCol) {
                            _hScrollbar.scrollBy(totalWidth(range4.startCol, range4.endCol, range.endCol));
                        }
                    }
                ...
                else if (range.startCol > range2.endCol &&
                    range.startCol < range4.startCol) {
                    _hScrollbar.scrollBy(-t.colsWidth(range.startCol, range4.startCol));
                }
            }
