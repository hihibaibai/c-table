...
function mousedownHandler(t, evt) {
    // console.log('evt:', evt);
    ...
    if (_selector && viewport) {
      ...
      // console.log(vcell);
        if (vcell) {
            ...
            selector.reset(t);
            selector.bindMousemove(t, (row, col) => {
                selector.unionRange(t, row, col);
            }, (s) => s.currentRange);
        }
    }
}
function mousemoveHandler(t, evt) {
    ...
    if (code === 'Enter' && !ctrlKey && !metaKey && !altKey) {
        ...
        if (t._copyable && _selector) {
            ...
            if (range) {
                ...
                navigator.clipboard.write([new ClipboardItem(items)]).then(() => console.log('clipboard has writed success'), (e) => console.log('clipboard has wirted failure: ', e));
            }
        }
    }
...
function toClipboardTextFrom(t, from) {
...
function toArraysFromClipboardText(text) {
    const arrays = [];
