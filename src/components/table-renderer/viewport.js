...
export default class Viewport {
    constructor(render) {
        ...
        // 1, 2-1, 2-3, 3,
        this.headerAreas = [
            Area.create(0, area1.range.startCol, _colHeader.rows - 1, area1.range.endCol, area4.x, 0, area4.width, 0, getColHeaderRow, getColWidth),
            Area.create(0, area2.range.startCol, _colHeader.rows - 1, area2.range.endCol, area2.x, 0, area2.width, 0, getColHeaderRow, getColWidth),
            Area.create(area2.range.startRow, 0, area2.range.endRow, _rowHeader.cols - 1, 0, area2.y, 0, area2.height, getRowHeight, getRowHeaderCol),
            Area.create(area3.range.startRow, 0, area3.range.endRow, _rowHeader.cols - 1, 0, area4.y, 0, area4.height, getRowHeight, getRowHeaderCol),
        ];
    }
    inAreas(row, col) {
    ...
    cellAt(x, y) {
        ...
        if (y < a2.y) {
            return Object.assign({ placement: 'col-header' }, (ha21.containsx(x) ? ha21 : ha1).cellAt(x, y));
        }
        for (let a of this.areas) {
            if (a.contains(x, y)) {
                return Object.assign({ placement: 'body' }, a.cellAt(x, y));
            }
        }
        return null;
    }
