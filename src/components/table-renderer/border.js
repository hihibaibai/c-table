...
export function borderRanges(area, [ref, type, ...borderOther], areaMerges) {
    // render borders
    const ret = [];
                    ...
                    bRange.difference(merge).forEach((it) => {
                        if (it.intersects(area.range)) {
                            ...
                            if (type === 'inside' || type === 'horizontal') {
                                if (it.startRow < merge.startRow &&
                                    it.endRow < merge.startRow) {
                                    // top
                                    ret.push([it, borderRect, 'bottom']);
                                }
                                else if (it.startRow > merge.startRow &&
                                    it.endRow > merge.startRow) {
                                    // bottom
