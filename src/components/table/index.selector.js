...
function reset(t) {
  const {_selector, _overlayer, toolbarStyle} = t;
...
function move(t, reselect, direction, step) {
  var _a;
  if (moveAutofill(t, direction))
    return;
  const {_selector, _data} = t;
  ...
  if (_selector && viewport) {
    ...
    if (_focusRange) {
      ...
      const oldCurrentRange = (_a = _selector.currentRange) === null || _a === void 0 ? void 0 : _a.clone();
      if (step) {
        if (direction === 'up') {
...
// bind mouse select
function bindMousemove(t, moveChange, changedRange, upAfter = (s) => {}) {
  ...
  if (_placement !== 'all') {
    ...
    const moveHandler = (e) => {
      ...
      if (target.tagName === 'CANVAS') {
        const c1 = (_b = _renderer.viewport) === null || _b === void 0 ? void 0 : _b.cellAt(x1, y1);
        if (c1) {
          let {row, col} = c1;
